...
namespace VoronoiEngine
{
	public class VoronoiGraph
	{
		...
		public static VoronoiGraph FromDCEL(DCEL dcel)
		{
			...
			foreach (var v in dcel.vertexList)
			{
				...
				f.id = voronoiGraph.faceList.Count + 1;
				voronoiGraph.faceList.Add(f);
				voronoiGraph.id_face[f.id] = f;
				voronoiGraph.dcelVertexID_faceID[v.id] = f.id;
				f.center = v.position;
			}
			...
			foreach (var f in dcel.faceList)
			{
				...
				voronoiV0.id = voronoiGraph.vertexList.Count + 1;
				voronoiV0.position = new Vector2(c0.x, c0.y);
				voronoiGraph.vertexList.Add(voronoiV0);
				...
				foreach (var e in edgeList)
				{
					if (e.twin?.face != null)
					{
						...
						if (c1.x > 0 && c1.x < 1 && c1.y > 0 && c1.y < 1)
						{
							...
							edge.ori = voronoiV0;
							voronoiV0.edge = edge;
							voronoiGraph.edgeList.Add(edge);

							if (MathUtility.GetPositionOrientationToEdge(new[] {e.ori.position.x, e.ori.position.y}, new []
							{
								new[] {c0.x, c0.y},
								new[] {c1.x, c1.y},
							}) > 0)
							{
								edge.face = voronoiGraph.id_face[voronoiGraph.dcelVertexID_faceID[e.ori.id]];
							}
							else
							{
								edge.face = voronoiGraph.id_face[voronoiGraph.dcelVertexID_faceID[e.twin.ori.id]];
							}
						}
					}
				}
	...
	public class DCELFace
	{
		...
		public Vector3 GetCircumcircle()
		{
			...
			var d = MathUtility.Determinant(new[]
			{
				new[] {x0, y0, 1},
				new[] {x1, y1, 1},
				new[] {x2, y2, 1},
			...
			var x = MathUtility.Determinant(new[]
			{
			...
			var y = MathUtility.Determinant(new[]
			{
